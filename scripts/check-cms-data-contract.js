...
const expected = new Set([
    ...homeContract.scalar,
    ...homeContract.objects,
    ...homeContract.lists,
    ...homeContract.collections,
]);
const missing = [...expected].filter((key) => demo[key] === undefined);
if (missing.length) {
    console.error(
        `CMS data contract: field demo hilang: ${missing.join(', ')}`,
    );
    process.exit(1);
}
...
for (const [collection, fields] of Object.entries(collectionFields)) {
    ...
    for (const [index, row] of rows.entries()) {
        for (const field of fields) {
            if (!(field in row))
                errors.push(`${collection}[${index}].${field}`);
        }
    }
}
...
if (errors.length) {
    console.error(
        `CMS data contract: field collection hilang:\n- ${errors.join('\n- ')}`,
    );
    process.exit(1);
}
