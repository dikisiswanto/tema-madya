...
const iconSource = await readFile(
    path.join(root, 'src', 'js', 'icons.js'),
    'utf8',
);
...
for (const name of iconNames) {
    if (!hasIcon(name))
        failures.push(`Missing bundled icon definition: ${name}`);
}
...
for (const field of dynamicIconFields)
    if (demoData[field] && !hasIcon(demoData[field]))
        failures.push(
            `Missing bundled icon definition: ${field}=${demoData[field]}`,
        );
for (const collection of ['programs', 'extracurriculars']) {
    for (const item of Array.isArray(demoData[collection])
        ...
        if (item.icon && !hasIcon(item.icon))
            failures.push(
                `Missing bundled icon definition: ${collection}.${item.icon}`,
            );
    }
}
