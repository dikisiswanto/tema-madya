...
const safe = (value, fallback = 'item') =>
    String(value || fallback).replace(/[^a-zA-Z0-9_-]/g, '-') || fallback;
const menuUrl = (item) =>
    item?.section_key
        ? `/#${String(item.section_key).replace(/^#/, '')}`
        : item?.url || '#';
const targetAttr = (item) =>
    item?.target === '_blank'
...
function renderDesktop(items, level = 0) {
    return items
        .map((item, index) => {
            ...
            if (!children.length)
                return `<li data-nav-item data-nav-depth="${level}" data-open="false" data-nav-leaf><a class="desktop-nav-link" href="${esc(url)}" data-spa-link${targetAttr(item)}>${title}</a></li>`;
...
function renderMobileLevel(item, key) {
    ...
    ${children
        .map((child, index) => {
            ...
            if (hasChildren)
                return `<button class="mobile-menu-trigger" type="button" data-mobile-trigger="${childKey}" aria-expanded="false" aria-controls="mobile-navigation-panel-${childKey}"><span>${childTitle}</span><span class="menu-arrow" aria-hidden="true"><i data-lucide="arrow-right"></i></span></button>`;
...
export function hydratePlaygroundNavigation() {
    ...
    const mobileRoot = document.querySelector(
        '.mobile-level[data-mobile-level="root"]',
    );
    ...
    mobileRoot.innerHTML = `<div class="mobile-nav-intro"><span class="eyebrow">Navigasi</span><p data-demo-mobile-intro>${esc(state.site_description || 'Temukan informasi sekolah berdasarkan kebutuhan Anda.')}</p></div>${tree
        .map((item, index) => {
            ...
            if (children.length)
                return `<button class="mobile-menu-trigger" type="button" data-mobile-trigger="${key}" aria-expanded="false" aria-controls="mobile-navigation-panel-${key}"><span>${title}</span><span class="menu-arrow" aria-hidden="true"><i data-lucide="arrow-right"></i></span></button>`;
