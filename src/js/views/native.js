...
function pageHeader(
    ...
    const safeIcon = String(fallbackIcon || 'school').replace(
        /[^a-z0-9-]/gi,
        '',
    );
    const fallback = imageUrl
        ? ''
        : `<div class="page-hero-icon-fallback" aria-hidden="true"><i class="fas fa-${safeIcon}"></i></div>`;
    return `<header class="page-hero${imageUrl ? ' page-hero-has-image' : ''}"${imageUrl ? ` style="--page-hero-image:url('${esc(imageUrl)}')"` : ''}><div class="page-hero-backdrop" aria-hidden="true"></div>${fallback}<div class="theme-container page-hero-inner"><nav class="breadcrumb page-breadcrumb" aria-label="Jejak navigasi"><a href="/">Beranda</a><span aria-hidden="true">/</span><span aria-current="page">${esc(title)}</span></nav><p class="eyebrow">${esc(eyebrow)}</p><h1>${esc(title)}</h1>${description ? `<p>${esc(description)}</p>` : ''}</div></header>`;
}
...
export function renderNews(state, container) {
    ...
    container.innerHTML = `${pageHeader('Berita & Artikel', 'Berita', 'Informasi terbaru seputar kegiatan, prestasi, dan program di sekolah.', hero, 'newspaper')}
...
export function renderArticle(state, slug, container) {
    ...
    if (!post) {
        container.innerHTML = `${pageHeader('Berita', 'Berita tidak ditemukan', 'Halaman yang Anda cari belum tersedia.', state.about?.hero_image || state.about?.image || '', 'newspaper')}<section class="madya-section"><div class="theme-container empty-state"><p>Berita yang diminta tidak tersedia.</p><a class="button" href="/news">Kembali ke berita</a></div></section>`;
        return null;
    }
    ...
    container.innerHTML = `${pageHeader('Detail', 'Berita', 'Informasi terbaru seputar kegiatan, prestasi, dan program di sekolah.', hero, 'newspaper')}<section class="madya-section article-detail-section"><div class="theme-container article-detail-layout"><article class="article-main"><header class="article-header"><span class="article-category">${esc(
...
export function renderDownloads(state, container) {
    ...
    const slug = (x) =>
        String(x || '')
            ...
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/^-|-$/g, '');
    const hero = state.about?.hero_image || '';
...
export function renderStaticPage(state, slug, container) {
    ...
    if (!page) {
        container.innerHTML = `${pageHeader('Informasi', 'Halaman tidak ditemukan', 'Halaman yang Anda cari belum tersedia.', state.about?.hero_image || state.about?.image || '', 'file-text')}<section class="madya-section"><div class="theme-container empty-state"><p>Halaman yang diminta tidak tersedia.</p><a class="button" href="/">Kembali ke beranda</a></div></section>`;
        return;
    }
...
export function renderContact(state, container) {
    ...
    container.innerHTML = `${pageHeader(
        'Hubungi Sekolah',
        'Kontak Sekolah',
        'Kami siap membantu Anda. Hubungi kami untuk informasi, kerja sama, atau layanan sekolah lainnya.',
        hero,
        'phone',
    )}<section class="contact-overlap-section madya-section"><div class="theme-container">
      <div class="contact-summary contact-summary-four -mr-2">
        <div class="contact-summary-item"><span class="contact-summary-icon">${iconMarkup('map-pin')}</span><div><strong>Alamat</strong><small>${esc(state.contact_address || '—')}</small></div></div>
        <div class="contact-summary-item"><span class="contact-summary-icon">${iconMarkup('phone')}</span><div><strong>Telepon</strong><small>${esc(state.contact_phone || '—')}</small></div></div>
        <div class="contact-summary-item"><span class="contact-summary-icon">${iconMarkup('mail')}</span><div><strong>Email</strong><small>${esc(state.contact_email || '—')}</small></div></div>
        <div class="contact-summary-item"><span class="contact-summary-icon">${iconMarkup('clock-3')}</span><div><strong>Jam Layanan</strong><small>${esc(state.contact_hours || '—')}</small></div></div>
      </div>
      <div class="contact-main-grid contact-main-grid-reference">
        <section class="contact-info-panel contact-reference-card">
          <div class="contact-info-heading"><p class="eyebrow">Lokasi Sekolah</p><h2>Temukan kami di lokasi resmi.</h2></div>
          <div class="contact-location-block">
            <div class="contact-map-wrap">${mapAddress ? `<iframe title="Peta lokasi sekolah" loading="lazy" src="https://www.google.com/maps?q=${mapAddress}&output=embed" referrerpolicy="no-referrer-when-downgrade"></iframe>` : `<div class="contact-map-fallback">${iconMarkup('map-pin')}<span>Alamat sekolah belum tersedia untuk menampilkan peta.</span></div>`}</div>
            ${mapAddress ? `<a class="contact-map-link" href="https://www.google.com/maps/search/?api=1&query=${mapAddress}" target="_blank" rel="noopener noreferrer">Buka di Google Maps ${iconMarkup('arrow-up-right')}</a>` : ''}
          </div>
        </section>
        <section class="contact-form-card">
          <p class="eyebrow">Kirim Pesan</p><h2>Hubungi tim sekolah.</h2>
          <p class="contact-form-intro">Isi formulir di bawah ini, tim kami akan segera merespons pesan Anda.</p>
          <form class="contact-form-grid" action="${esc(state.urls?.contact_send || '/contact/send')}" method="post" onsubmit="return false">
            <div class="form-field"><label for="contact-name">Nama Lengkap <span>*</span></label><input id="contact-name" name="name" placeholder="Masukkan nama lengkap Anda" required></div>
            <div class="form-field"><label for="contact-email">Email <span>*</span></label><input id="contact-email" name="email" type="email" placeholder="Masukkan email Anda" required></div>
            <div class="form-field form-full"><label for="contact-subject">Subjek <span>*</span></label><input id="contact-subject" name="subject" placeholder="Apa yang ingin Anda sampaikan?" required></div>
            <div class="form-field form-full"><label for="contact-message">Pesan <span>*</span></label><textarea id="contact-message" name="message" placeholder="Tulis pesan Anda di sini..." required maxlength="5000"></textarea><small class="form-counter">Maks. 5000 karakter</small></div>
            <div class="contact-form-note">${iconMarkup('shield-check')}<span>Data Anda hanya digunakan untuk menindaklanjuti pesan dan kebutuhan komunikasi dengan sekolah.</span></div>
            <div><button class="button button-contact-submit" type="submit">${iconMarkup('send')}Kirim Pesan</button></div>
          </form>
          <p class="contact-response-note">Pesan akan dibalas pada jam kerja (${esc(state.contact_hours || 'jam layanan sekolah')}).</p>
        </section>
      </div>
    </div></section>`;
}
