...
import { articleReadMinutes, newsArchive, newsCategories, newsTags, popularNews, relatedNews } from '../data/derived.js';
...
export function renderNews(state,container){
  ...
  const posts=allPosts.filter((post)=>{
    const haystack=[post.title,post.excerpt,post.content,post.category,post.author].filter(Boolean).join(' ').toLowerCase();
    const categoryMatch=!category || String(post.category||'').toLowerCase()===category;
    const monthMatch=!month || String(post.published_at||post.created_at||'').startsWith(month) || String(post.published_at||post.created_at||'').slice(0,7)===month;
    return (!search || haystack.includes(search)) && categoryMatch && monthMatch;
  });
  ...
  const categoryLabels=counts.length?counts.map(x=>x.name):['Prestasi','Kegiatan','Pengumuman','Akademik','Artikel'];
...
export function renderDownloads(state,container){
  const items=Array.isArray(state.downloads)?state.downloads.filter(x=>Number(x.show??1)!==0):[];
  const cats=[...new Set(items.map(x=>x.category).filter(Boolean))];
  const ext=x=>String(x.extension||((x.url||'').split('?')[0].split('.').pop())||'PDF').toUpperCase();
  const pdfCount=items.filter(x=>ext(x)==='PDF').length;
  const otherCount=Math.max(0,items.length-pdfCount);
  const slug=x=>String(x||'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
  const hero=state.about?.hero_image||'/themes/madya/assets/generated/hero-campus.jpg';
  const stat=(icon,value,label)=>`<div class="download-stat"><span class="download-stat-icon">${iconMarkup(icon)}</span><span><strong>${esc(value)}</strong><small>${esc(label)}</small></span></div>`;
  const itemRow=x=>{const e=ext(x);const icon=e==='PDF'?'file-text':(e==='XLS'||e==='XLSX'?'clipboard-text':(e==='DOC'||e==='DOCX'?'file-text':'file-text'));return `<a class="document-row-reference" href="${esc(x.url||'#')}" target="_blank" rel="noopener noreferrer"><span class="document-file-type document-file-type-${esc(e.toLowerCase())}">${iconMarkup(icon)}<b>${esc(e)}</b></span><span class="document-main-reference"><strong>${esc(x.title||'Dokumen')}</strong>${x.description?`<small>${esc(x.description)}</small>`:''}<span class="document-meta-reference">${iconMarkup('download')}${esc(x.file_size||'Ukuran tidak tersedia')}</span></span><span class="document-download-button">${iconMarkup('download')}Unduh</span></a>`;};
  const groups=cats.map(cat=>`<section class="document-group document-group-reference" id="doc-${slug(cat)}"><div class="document-group-heading"><div><p class="eyebrow">Koleksi</p><h2>${esc(cat)}</h2></div><span>${items.filter(x=>x.category===cat).length} dokumen</span></div><div class="document-list-reference">${items.filter(x=>x.category===cat).map(itemRow).join('')}</div></section>`).join('');
...
export function renderStaticPage(state, slug, container) {
  ...
  const page = pages.find(item => String(item.slug || '').replace(/^\//,'') === String(slug || '').replace(/^\//,''));
  if (!page) {
  ...
  const docs = Array.isArray(state.downloads) ? state.downloads.filter(item => Number(item.show ?? 1) !== 0).slice(0, 3) : [];
...
export function renderContact(state,container){
  ...
  const social=state.social_instagram||state.social_facebook||state.social_youtube||'Kanal resmi sekolah';
  const about=state.about||{};
  const helpLinks=[['file-text','Pusat Download','Dokumen dan formulir resmi','/downloads'],['newspaper','Berita Sekolah','Informasi dan pengumuman','/news'],['school','Profil Sekolah','Kenali sekolah lebih dekat','/#profile'],['messages-square','Hubungi Kami','Kanal komunikasi resmi','/contact']];
