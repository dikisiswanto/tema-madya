...
function renderPrograms(state) {
    ...
    const cards = items.map((item, i) => `<article class="rich-program-card"><span class="rich-card-icon rich-data-icon">${iconMarkup(item.icon || ['atom', 'languages', 'dribbble'][i] || 'book-open')}</span><h3>${escapeHtml(item.title || 'Program Unggulan')}</h3><p>${escapeHtml(item.description || 'Program pengembangan potensi siswa.')}</p></article>`).join('');
    const extraCards = extras.map(item => {
        const iconColor = /^#[0-9a-f]{6}$/i.test(String(item.icon_color || '')) ? item.icon_color : '';
        const iconStyle = iconColor ? ` style="--rich-icon-color:${escapeHtml(iconColor)}"` : '';
...
function renderExtracurriculars(state) {
    const items = Array.isArray(state.extracurriculars) ? state.extracurriculars : [];
...
function renderTeachers(state) {
    ...
    const cards = items.map((item) => {
        ...
        return `<figure class="rich-person-card" data-filter-item data-filter-group="teacher-role" data-filter-value="${escapeHtml(group)}"><div class="rich-person-media"><img src="${escapeHtml(item.photo || '')}" alt="${escapeHtml(item.name || 'Tenaga pengajar')}" loading="lazy" decoding="async"></div><figcaption><span class="rich-person-role">${escapeHtml(group)}</span><strong>${escapeHtml(item.name || 'Tenaga pengajar')}</strong><span>${escapeHtml(role)}</span></figcaption></figure>`;
    }).join('');
...
function renderAchievements(state) {
    ...
    const cards = items.map((item, index) => {
        ...
        const color = ['#2563eb', '#16805f', '#a56a08', '#7c3aed'][index % 4];
...
function renderEvents(state) {
    ...
    const months = [...new Set(items.map(item => item.event_date ? new Date(item.event_date).toLocaleString('id-ID', { month: 'long', year: 'numeric' }) : '').filter(Boolean))];
    const now = new Date();
    ...
    const cards = items.map(item => {
        ...
        const month = d ? new Date(d).toLocaleString('id-ID', { month: 'long', year: 'numeric' }) : '';
