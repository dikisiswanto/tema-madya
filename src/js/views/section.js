...
function renderProfile(state) {
    ...
    const downloads = Array.isArray(state.downloads)
        ? state.downloads
    ...
    const principalCard =
        principal.name || principal.quote || principal.photo
    ...
    return (
        pageHeader('profile', state) +
        `<section class="section static-page-section"><div class="theme-container static-page-layout profile-no-sidebar">${main}</div></section>`
    );
}
function renderPrograms(state) {
    ...
    const cards = items
        .map(
            (item, i) =>
                `<article class="rich-program-card"><span class="rich-card-icon">${iconMarkup(item.icon || ['atom', 'languages', 'dribbble'][i] || 'book-open')}</span><h3>${escapeHtml(item.title || 'Program Akademik')}</h3><p>${escapeHtml(item.description || '')}</p>${item.link_url ? `<a class="text-link" href="${escapeHtml(item.link_url)}">${escapeHtml(item.link_text || 'Selengkapnya')} ${iconMarkup('arrow-right')}</a>` : ''}</article>`,
        )
        .join('');
    const extraCards = extras
        .map(
            (item) =>
                `<article class="rich-program-row"><span class="rich-card-icon">${iconMarkup(item.icon || 'users-round')}</span><div><h3>${escapeHtml(item.title || 'Kegiatan')}</h3><p>${escapeHtml(item.description || '')}</p></div></article>`,
        )
        .join('');
    ...
    return (
        pageHeader('programs', state) + richLayout('programs', state, content)
    );
}

function renderExtracurriculars(state) {
    return (
        pageHeader('extracurriculars', state) +
        richLayout(
            'extracurriculars',
            state,
            `<main class="rich-main"><div class="rich-card-grid">${(state.extracurriculars || []).map((item) => richCollectionCard(item, item.icon || 'star')).join('') || emptyState('Belum ada kegiatan.')}</div></main>`,
        )
    );
}
...
function renderTeachers(state) {
    ...
    const cards = items
        .map(
            (item) =>
                `<figure class="rich-person-card"><img src="${escapeHtml(item.photo || '')}" alt="${escapeHtml(item.name || 'Tenaga pengajar')}" loading="lazy" decoding="async"><figcaption><strong>${escapeHtml(item.name || 'Tenaga pengajar')}</strong><span>${escapeHtml(item.role || 'Guru')}</span></figcaption></figure>`,
        )
        .join('');
    ...
    return (
        pageHeader('teachers', state) + richLayout('teachers', state, content)
    );
}
...
function renderAchievements(state) {
    ...
    return (
        pageHeader('achievements', state) +
        richLayout('achievements', state, content)
    );
}
...
function renderTestimonials(state) {
    ...
    return (
        pageHeader('testimonials', state) +
        richLayout('testimonials', state, content)
    );
}
...
function renderEvents(state) {
    ...
    const content = `<main class="rich-main"><div class="rich-toolbar"><div class="rich-filters"><button class="is-active" type="button">Semua</button><button type="button">Mendatang</button><button type="button">Berlangsung</button><button type="button">Selesai</button></div><select class="rich-month" aria-label="Bulan"><option value="">Semua bulan</option>${months.map((month) => `<option>${escapeHtml(month)}</option>`).join('')}</select></div><div class="rich-event-list">${
        (state.events || [])
            .map((item) => {
                const d = item.event_date || '';
                return `<article class="rich-event-row"><time datetime="${escapeHtml(d)}"><b>${d ? new Date(d).getDate() : '—'}</b><span>${d ? new Date(d).toLocaleString('id-ID', { month: 'short' }).toUpperCase() : ''}</span></time><div><h2>${escapeHtml(item.title || 'Kegiatan sekolah')}</h2><p>${escapeHtml([item.event_time, item.location].filter(Boolean).join(' · '))}</p><small>${escapeHtml(item.description || '')}</small></div><span class="rich-event-tag">${escapeHtml(item.status || 'upcoming')}</span></article>`;
            })
            .join('') || emptyState('Belum ada agenda.')
    }</div></main>`;
...
function richCollectionCard(item, icon) {
    return `<article class="rich-program-card"><span class="rich-card-icon">${iconMarkup(icon)}</span><h2>${escapeHtml(item.title || 'Kegiatan')}</h2><p>${escapeHtml(item.description || '')}</p></article>`;
}
