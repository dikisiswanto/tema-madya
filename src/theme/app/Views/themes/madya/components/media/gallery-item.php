...
<figure class="gallery-home-item gallery-home-item-<?= $index % 5 ?>">
    <?php if ($src): ?><img src="<?= esc($src) ?>" width="<?= esc($item['image_width'] ?? 1600) ?>" height="<?= esc($item['image_height'] ?? 1000) ?>" alt="<?= esc($item['title'] ?? 'Dokumentasi sekolah') ?>" loading="lazy" decoding="async">
