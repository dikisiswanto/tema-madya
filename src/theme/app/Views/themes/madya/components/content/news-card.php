...
<article class="news-card<?= $featured ? ' news-card-featured' : '' ?>">
    <?php if (!empty($post['image'])): ?>
        <a class="news-card-media" href="<?= esc($href) ?>" aria-label="Baca <?= esc($post['title'] ?? 'berita') ?>">
            ...
            <?php if (!empty($post['category'])): ?><span class="news-card-category"><?= esc($post['category']) ?></span><?php endif; ?>
        </a>
    <?php endif; ?>
    <div class="news-card-body">
        ...
        <div class="news-card-footer">
            <div class="news-card-meta" aria-label="Metadata berita">
                <?php if (!empty($post['author'])): ?><span><i data-lucide="user" aria-hidden="true"></i><?= esc($post['author']) ?></span><?php endif; ?>
                <?php if (isset($post['view_count'])): ?><span><i data-lucide="eye" aria-hidden="true"></i><?= esc(number_format((int)$post['view_count'], 0, ',', '.')) ?> kali dibaca</span><?php endif; ?>
            </div>
