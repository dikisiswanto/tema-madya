...
if (!isset($downloads) && class_exists('\App\Models\DownloadModel')) {
    $downloads = (new \App\Models\DownloadModel())->getVisible();
}
    ...
    <div class="theme-container home-middle-stack">
        <div class="home-middle-grid home-middle-grid-profile">
            <article class="principal-panel">
                ...
                <div class="principal-message">
                    ...
                    <blockquote>“<?= esc($principalData['welcome_message'] ?? 'Selamat datang di sekolah kami.') ?>”</blockquote>
                    <p class="principal-description"><?= esc('') ?></p>
                    <strong><?= esc($principalData['name'] ?? 'Kepala Sekolah') ?></strong>
                    <span><?= esc($principalData['role_title'] ?? 'Kepala Sekolah') ?></span>
                    <?php
                    $principalFacts = [
                ...
                <?php if ($programItems): ?><div class="program-showcase-grid">
                    <?php foreach (array_slice($programItems, 0, 3) as $index => $item): ?>
                        <article class="program-showcase-card">
                            <div class="program-showcase-media"><img src="<?= esc($generated('program-' . (['science','language','students'][$index % 3]) . '.jpg')) ?>" width="800" height="520" alt="" loading="lazy" decoding="async"><span class="program-showcase-icon" aria-hidden="true"><i data-lucide="<?= esc($item['icon'] ?? 'star') ?>"></i></span></div>
                            <div class="program-showcase-body"><h3><?= esc($item['title'] ?? 'Program') ?></h3><p><?= esc($item['description'] ?? $item['excerpt'] ?? '') ?></p></div>
                        </article>
                    <?php endforeach; ?>
        ...
        <div class="home-middle-grid home-middle-grid-people" id="teachers">
            <?php if ($teacherItems): ?><article class="middle-panel">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($teacherMeta['title']) ?></p><h2><?= esc($teacherMeta['subtitle'] ?: 'Tenaga pengajar pilihan.') ?></h2></div><a class="text-link" href="#teachers">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
            ...
            <?php if ($achievementItems): ?><article class="middle-panel" id="achievements">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($achievementMeta['title']) ?></p><h2><?= esc($achievementMeta['subtitle'] ?: 'Mengukir prestasi di berbagai bidang.') ?></h2></div><a class="text-link" href="#achievements">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
        ...
        <div class="home-middle-grid home-middle-grid-updates" id="updates">
            ...
            <?php if ($eventItems): ?><article class="middle-panel">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($eventMeta['title']) ?></p><h2><?= esc($eventMeta['subtitle'] ?: 'Agenda penting sekolah.') ?></h2></div><a class="text-link" href="#events">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
        ...
        <div class="home-middle-grid home-middle-grid-community" id="gallery">
            <?php if ($galleryItems): ?><article class="middle-panel">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($galleryMeta['title']) ?></p><h2><?= esc($galleryMeta['subtitle'] ?: 'Momen terbaik kami.') ?></h2></div><a class="text-link" href="#gallery">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
                <div class="gallery-home-grid gallery-home-grid-compact"><?php foreach (array_slice($galleryItems, 0, 4) as $index => $item): ?><a class="gallery-home-item gallery-home-item-<?= $index ?>" href="#gallery"><img src="<?= esc($item['image'] ?? $generated('gallery-' . (['students','class','library'][$index % 3]) . '.jpg')) ?>" width="700" height="520" alt="<?= esc($item['caption'] ?? 'Dokumentasi sekolah') ?>" loading="lazy" decoding="async"></a><?php endforeach; ?></div>
            </article><?php endif; ?>
            <?php if ($testimonialItems): ?><article class="middle-panel">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($testimonialMeta['title']) ?></p><h2><?= esc($testimonialMeta['subtitle'] ?: 'Apa kata mereka tentang sekolah?') ?></h2></div><a class="text-link" href="#testimonials">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
...
<?php if ($downloadMeta['show']): ?><section class="home-section section-soft" id="documents" aria-labelledby="documents-title"><div class="theme-container documents-spmb-grid">
    <div><div class="section-head-row compact"><div><p class="section-kicker"><?= esc($downloadMeta['title']) ?></p><h2 id="documents-title"><?= esc($downloadMeta['subtitle'] ?: 'Unduh dokumen dan formulir penting.') ?></h2></div><a class="text-link" href="<?= base_url('downloads') ?>">Lihat Semua Dokumen <i data-lucide="arrow-right" aria-hidden="true"></i></a></div><div class="document-home-list"><?php foreach ($downloadItems as $item): ?><a href="<?= esc($item['url'] ?? '#') ?>" class="document-home-row"><span class="document-file-icon"><i data-lucide="file-text" aria-hidden="true"></i></span><span><strong><?= esc($item['title'] ?? 'Dokumen') ?></strong><small><?= esc(strtoupper(pathinfo((string)($item['url'] ?? ''), PATHINFO_EXTENSION) ?: 'FILE')) ?> · <?= esc($item['file_size'] ?? $item['size'] ?? '') ?></small></span><i data-lucide="arrow-up-right" aria-hidden="true"></i></a><?php endforeach; ?></div></div>
    <aside class="spmb-home-banner"><div><p class="section-kicker">SPMB Online</p><h2>Berikan langkah terbaik untuk masa depan gemilang.</h2><p>Pendaftaran peserta didik baru melalui kanal resmi sekolah.</p><a class="button button-accent" href="<?= esc($spmb_url ?? '#') ?>">Daftar Sekarang <i data-lucide="arrow-right" aria-hidden="true"></i></a></div><?php if ($heroImage): ?><img src="<?= esc($heroImage) ?>" width="800" height="600" alt="" loading="lazy" decoding="async"><?php endif; ?></aside>
