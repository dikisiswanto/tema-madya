...
$heroImage = $aboutData['hero_image'] ?? $aboutData['image'] ?? $aboutData['image_url'] ?? $generated('hero-campus-new.jpg');
...
<?php if ($counterItems): ?><section class="home-stat-section" aria-label="Statistik sekolah"><div class="theme-container"><div class="home-stat-grid">
    <?php foreach ($counterItems as $stat): ?><div class="home-stat-item"><?= $this->include('themes/madya/components/ui/icon', ['name' => $stat['icon'] ?? 'sparkles']) ?><div><strong><?= esc($stat['number'] ?? '') ?><?= esc($stat['suffix'] ?? '') ?></strong><span><?= esc($stat['label'] ?? '') ?></span></div></div><?php endforeach; ?>
    ...
    <div class="theme-container home-middle-stack">
        <div class="home-middle-grid home-middle-grid-profile">
            <article class="principal-panel">
                <div class="principal-photo">
                    <img src="<?= esc($mediaUrl($principalData['photo'] ?? '', 'principal') ?: $generated('principal.jpg')) ?>" width="<?= esc($principalData['image_width'] ?? 900) ?>" height="<?= esc($principalData['image_height'] ?? 1100) ?>" alt="<?= esc($principalData['name'] ?? 'Kepala sekolah') ?>" loading="lazy" decoding="async">
                </div>
                <div class="principal-message">
                    <p class="section-kicker">Sambutan Kepala Sekolah</p>
                    <blockquote>“<?= esc($principalData['welcome_message'] ?? $principalData['quote'] ?? 'Selamat datang di sekolah kami.') ?>”</blockquote>
            ...
            <article class="program-panel" id="programs" aria-labelledby="programs-title">
                <div class="section-head-row compact">
                    <div><p class="section-kicker"><?= esc($programMeta['title']) ?></p><h2 id="programs-title"><?= esc($programMeta['title']) ?></h2><p class="section-description"><?= esc($programMeta['subtitle']) ?></p></div>
                    <a class="text-link" href="<?= base_url('/#programs') ?>">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a>
                </div>
                <?php if ($programItems): ?><div class="program-showcase-grid">
                    <?php foreach (array_slice($programItems, 0, 3) as $index => $item): ?>
                        <article class="program-showcase-card">
                            <div class="program-showcase-icon" aria-hidden="true"><?= $this->include('themes/madya/components/ui/icon', ['name' => $item['icon'] ?? 'star', 'class' => 'program-icon']) ?></div>
                            <div class="program-showcase-body"><h3><?= esc($item['title'] ?? 'Program Unggulan') ?></h3><p><?= esc($item['description'] ?? $item['excerpt'] ?? 'Program pengembangan potensi siswa.') ?></p></div>
                        </article>
                    <?php endforeach; ?>
                </div><?php else: ?><div class="empty-inline">Belum ada program unggulan.</div><?php endif; ?>
                <?php if ($extraItems): ?><div class="extra-inline" id="extracurriculars"><div class="subsection-label"><span>Ekstrakurikuler</span><a href="<?= base_url('/#extracurriculars') ?>">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div><div class="extra-inline-list"><?php foreach (array_slice($extraItems, 0, 4) as $item): $iconColor = preg_match('/^#[0-9a-f]{6}$/i', (string)($item['icon_color'] ?? '')) ? $item['icon_color'] : ''; ?><a class="extra-inline-item" href="<?= base_url('/#extracurriculars') ?>"><span class="extra-inline-icon"<?= $iconColor ? ' style="--rich-icon-color:' . esc($iconColor) . '"' : '' ?>><?= $this->include('themes/madya/components/ui/icon', ['name' => $item['icon'] ?? 'users-round', 'color' => $iconColor]) ?></span><span><?= esc($item['title'] ?? 'Kegiatan') ?></span></a><?php endforeach; ?></div></div><?php endif; ?>
            </article>
        </div>

        <div class="home-middle-grid home-middle-grid-people" id="teachers">
            <?php if ($teacherItems): ?><article class="middle-panel">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($teacherMeta['title']) ?></p><h2><?= esc($teacherMeta['subtitle'] ?: 'Tenaga pengajar pilihan.') ?></h2></div><a class="text-link" href="<?= base_url('/#teachers') ?>">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
                <div class="teacher-grid">
                    <?php foreach (array_slice($teacherItems, 0, 4) as $index => $item): ?><article class="teacher-card"><img src="<?= esc($mediaUrl($item['photo'] ?? '', 'teachers') ?: $generated('teacher-' . (($index % 4) + 1) . '.jpg')) ?>" width="900" height="1100" alt="<?= esc($item['name'] ?? 'Tenaga pendidik') ?>" loading="lazy" decoding="async"><div><strong><?= esc($item['name'] ?? 'Tenaga pendidik') ?></strong><span><?= esc($item['role'] ?? 'Tenaga pendidik') ?></span></div></article><?php endforeach; ?>
                </div>
            </article><?php endif; ?>
            <?php if ($achievementItems): ?><article class="middle-panel" id="achievements">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($achievementMeta['title']) ?></p><h2><?= esc($achievementMeta['subtitle'] ?: 'Mengukir prestasi di berbagai bidang.') ?></h2></div><a class="text-link" href="<?= base_url('/#achievements') ?>">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
                <div class="achievement-list">
                    <?php foreach (array_slice($achievementItems, 0, 3) as $item): ?><article class="achievement-list-row"><span class="achievement-list-icon"><i data-lucide="trophy" aria-hidden="true"></i></span><div><strong><?= esc(($item['achievement'] ?? 'Prestasi siswa') . (!empty($item['student_name']) ? ' — ' . $item['student_name'] : '')) ?></strong><small><?= esc(trim(($item['level'] ?? '') . (!empty($item['class_name']) ? ' · ' . $item['class_name'] : ''))) ?></small></div><time><?= esc($item['year'] ?? '') ?></time></article><?php endforeach; ?>
                </div>
            </article><?php endif; ?>
        </div>
        ...
        <div class="home-middle-grid home-middle-grid-updates" id="updates">
            <?php if ($newsItems): ?><article class="middle-panel">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($newsMeta['title']) ?></p><h2><?= esc($newsMeta['subtitle'] ?: 'Informasi terkini seputar kegiatan sekolah.') ?></h2></div><a class="text-link" href="<?= base_url('news') ?>">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
            ...
            <?php if ($eventItems): ?><article class="middle-panel">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($eventMeta['title']) ?></p><h2><?= esc($eventMeta['subtitle'] ?: 'Agenda penting sekolah.') ?></h2></div><a class="text-link" href="<?= base_url('/#events') ?>">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
                <div class="agenda-list" id="events"><?php foreach (array_slice($eventItems, 0, 3) as $item): ?><article class="agenda-item"><time datetime="<?= esc($item['event_date'] ?? $item['date'] ?? '') ?>"><b><?= esc(!empty($item['event_date']) ? date('d', strtotime((string)$item['event_date'])) : '—') ?></b><span><?= esc(!empty($item['event_date']) ? strtoupper(date('M', strtotime((string)$item['event_date']))) : '') ?></span></time><div><h3><?= esc($item['title'] ?? 'Kegiatan sekolah') ?></h3><p><?= esc($item['event_time'] ?? '') ?><?= !empty($item['location']) ? ' · ' . esc($item['location']) : '' ?></p></div><i data-lucide="chevron-right" aria-hidden="true"></i></article><?php endforeach; ?></div>
            </article><?php endif; ?>
        </div>

        <div class="home-middle-grid home-middle-grid-community" id="gallery">
            <?php if ($galleryItems): ?><article class="middle-panel">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($galleryMeta['title']) ?></p><h2><?= esc($galleryMeta['subtitle'] ?: 'Momen terbaik kami.') ?></h2></div><a class="text-link" href="<?= base_url('/#gallery') ?>">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
                <div class="gallery-home-grid gallery-home-grid-compact"><?php foreach (array_slice($galleryItems, 0, 4) as $index => $item): ?><a class="gallery-home-item gallery-home-item-<?= $index ?>" href="<?= base_url('/#gallery') ?>"><img src="<?= esc($item['image'] ?? $generated('gallery-' . (['students','class','library'][$index % 3]) . '.jpg')) ?>" width="700" height="520" alt="<?= esc($item['caption'] ?? 'Dokumentasi sekolah') ?>" loading="lazy" decoding="async"></a><?php endforeach; ?></div>
            </article><?php endif; ?>
            <?php if ($testimonialItems): ?><article class="middle-panel">
                <div class="section-head-row compact"><div><p class="section-kicker"><?= esc($testimonialMeta['title']) ?></p><h2><?= esc($testimonialMeta['subtitle'] ?: 'Apa kata mereka tentang sekolah?') ?></h2></div><a class="text-link" href="<?= base_url('/#testimonials') ?>">Lihat Semua <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
                <div class="testimonial-feature" id="testimonials"><?php $testimonial = $testimonialItems[0]; ?><div class="testimonial-quote-mark">“</div><blockquote><?= esc($testimonial['quote'] ?? $testimonial['content'] ?? '') ?></blockquote><div class="testimonial-person"><img src="<?= esc($mediaUrl($testimonial['photo'] ?? '', 'testimonials') ?: $generated('testimonial-1.jpg')) ?>" width="80" height="80" alt="" loading="lazy" decoding="async"><span><strong><?= esc($testimonial['name'] ?? $testimonial['author'] ?? 'Warga sekolah') ?></strong><small><?= esc($testimonial['role'] ?? '') ?></small></span></div></div>
            </article><?php endif; ?>
        </div>
    </div>
</section>
...
<section class="home-section section-soft" id="spmb">
    <div class="theme-container">
        <aside class="spmb-home-banner">
            <div><p class="section-kicker">SPMB Online</p><h2>Pendaftaran peserta didik baru.</h2><p>Bergabung bersama kami dan raih masa depan gemilang.</p><a class="button button-accent" href="<?= esc($spmb_url) ?>">Daftar Sekarang <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
            <?php if ($heroImage): ?><img src="<?= esc($heroImage) ?>" width="800" height="600" alt="" loading="lazy" decoding="async"><?php endif; ?>
        </aside>
    </div>
</section>
<?php endif; ?>

<?php if ($contactMeta['show']): ?><section class="home-contact-strip" id="contact" aria-label="Kontak sekolah"><div class="theme-container contact-strip-grid">
